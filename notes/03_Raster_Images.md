...
图像中所有像素的 $\alpha$ 值可以存储在一个单独的灰度图像中，此时该图像被称为 alpha 掩码或透明度掩码。或者，该信息可以作为 RGB 图像的第四个通道存储，此时称为 alpha 通道，该图像可以称为 RGBA 图像。对于 8-bit 图像，每个像素占用32位，这在许多计算机架构中是一个方便的结构块。
